...
signal received_command(user, command, args)
...
func _handle_chat(input):
	...
	if message.left(1) == "!":
		var command_and_args = message.substr(1).split(" ")
		var command = command_and_args[0]

		command = command.strip_edges()
		command_and_args.remove(0)

		var args = []
		for arg in command_and_args:
			args.append(arg.strip_edges())

		emit_signal("received_command", user, command, args)
